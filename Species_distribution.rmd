...
habitat_preferences <- habitat_preferences %>%
  mutate(species_1001 = G.3.6. + G.4.3.,
         species_1090 = G.3.6. + 0.5 * G.3.5. + G.4.3.,
         species_1008 = G.3.6. + G.4.3. + G.3.2. + G.4.2.,
         species_1027 = G.3.6.,
         species_8330 = G.3.6. + G.4.3.,
         species_1170 = G.3.6. + G.3.5. + G.4.3.)

#weights of 0 and 1
habitat_preferences$habitat_preferences_species_1001 <- ifelse(habitat_preferences$G.3.6. > 0 || habitat_preferences$G.4.3. > 0, 1, 0)
...
train %>% left_join(habitat_preferences) %>% 
  fill(G.3.2.:species_1170, 0, .direction = "down") -> train
...
for (i in species_habitat_codes){
  ...
  train %>% select(-starts_with("species")) -> train_df
  #unique at the end is needed, otherwise it gives me "double column"
  train_df %>% left_join(train_species, by = "fid") %>% unique -> train_df
  
  train_df %>% select(-cellcode, -x, -morska_mea, - morska_med, -layer, -path, -geometry) -> train_df
  train_df %>% select(species_1090) %>% min_max_norm()
  ...
  model_2 <- glm(formula = pojava~bati_mean:bati_media+y, train_df, family = "binomial")
