...
layer_10x10 <- st_read(dsn)
#load the 1x1 layer
wfs_layer <- "dzzpnpis:hr1x1"
dsn <- paste0(wfs_url,"?", wfs_request, wfs_layer)
layer_1x1 <- st_read(dsn)
...
for (i in species_habitat_codes){
  ...
  threshold <- 4928506
  if (species_habitat_code == "1090" | species_habitat_code=="1027"){
...
df_combined %>% row.names() -> df_combined$rown
df_combined %>%
  mutate(group = floor(as.numeric(rown))) %>%
  group_by(group) %>%
  summarize(number = n(),
            area = sum(st_area(df_combined$geometry))) -> df_summary

# Summarize the data frame by counting the number of rows for each data frame
df_summary <- data.frame(count = sapply(df_list, nrow))

# Calculate the area of the polyggons
polygon_areas <- sapply(df_combined$geometry, function(x) st_area(x))
...
Now that I've done the predictions, summary of statistics is left to do for each shapefile in folder.
