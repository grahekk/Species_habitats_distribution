...
for (i in species_habitat_codes){
  ...
  predictors %>% filter(pred_norm > 0.42) %>% 
    st_write(paste0("./data/",species_habitat_code,"_",grid,"_predicted_FRR.shp"), append = F)
  #favourable reference population
  predictors %>% filter(round_p2 == 1) %>% 
    st_write(paste0("./data/",species_habitat_code,"_",grid,"_predicted_FRP.shp"), append = F)
}
