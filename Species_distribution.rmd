...
grid_with_stats <- over(grid_polygons_overlap, benthos_map, fn=function(x, y) {
  # Calculate the area of the benthos habitats in each polygon
  area <- sapply(unique(y$habitat), function(habitat) {
    sum(x[y$habitat == habitat,]$area)
  })
  area
})

# Bind the zonal statistics to the grid_polygons_overlap data frame
grid_polygons_overlap$benthos_habitat_area <- grid_with_stats

# Calculate the percentage of each benthos habitat in each polygon
grid_polygons_overlap$benthos_habitat_percentage <-
  grid_polygons_overlap$benthos_habitat_area / 
  sum(grid_polygons_overlap$benthos_habitat_area) * 100
...
5. habitat preferences for species are added
