...
grid_polygons_overlap_area_pct <- predictors_inersect$area / as.double(st_area(predictors)) %>% as.double
...
results <- data.frame(fid = predictors_inersect$fid,
                      ...
                      area_pct = grid_polygons_overlap_area_pct)
...
df_summed <- habitat_preferences %>% mutate_all(as.numeric)
df_summed <- habitat_preferences %>% mutate(G.3.2. = as.double(G.3.2.))
df_summed[3,2]

df_summed[] <- sapply(habitat_preferences, function(x) {
  if (is.list(x)) unlist(x) else x
})
habitat_preferences

?parse
df_summed <- habitat_preferences %>% mutate_all(~{
  if (length(eval(parse(text = .))) > 1) {
    sum(unlist())
  } else {
    .
  }
...
```{r}
# Assign weights for each species and habitat
habitat_preferences$G.3.6.
