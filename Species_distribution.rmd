...
Zonal statistics for habitat maps and area percentage of each habitat in each grid cell
...
area_percentage <- predictors_inersect$area / as.double(st_area(predictors)) %>% as.double

# create data frame with the results
results <- data.frame(fid = predictors_inersect$fid,
...
habitat_preferences <- results %>% pivot_wider(id_cols = c("fid"),
...
#log scale
habitat_preferences %>%
  mutate_at(vars(starts_with("area_pct")), funs(log10)) -> habitat_preferences

#weights of 0 and 1
habitat_preferences$habitat_preferences_1001 <- ifelse(habitat_preferences$G.3.6. > 0 | habitat_preferences$G.4.3. > 0, 1, 0)
habitat_preferences$habitat_preferences_1090 <- ifelse(habitat_preferences$G.3.6. > 0 | habitat_preferences$G.3.5. > 0, 1, 0)
habitat_preferences$habitat_preferences_1008 <- ifelse(habitat_preferences$G.3.6. > 0 | habitat_preferences$G.4.3. > 0 | habitat_preferences$G.3.2. > 0 || habitat_preferences$G.4.2. > 0, 1, 0)
habitat_preferences$habitat_preferences_1027 <- ifelse(habitat_preferences$G.3.6. > 0, 1, 0)
habitat_preferences$habitat_preferences_8330 <- ifelse(habitat_preferences$G.3.6. > 0 | habitat_preferences$G.4.3. > 0, 1, 0)
habitat_preferences$habitat_preferences_1170 <- ifelse(habitat_preferences$G.3.6. > 0 | habitat_preferences$G.3.5. > 0, 1, 0)

#assign  bathymetry preferences
...
#$habitat and bathymetry preferences
habitat_preferences %>% 
  mutate(habitat_preferences_1001 = list(ifelse(G.3.6. > 0 | G.4.3. > 0, 1, 0))) %>%
  mutate(habitat_preferences_1090 = list(ifelse(G.3.6. > 0 | G.3.5. > 0, 1, 0))) %>%
  mutate(habitat_preferences_1008 = list(ifelse(G.3.6. > 0 | G.4.3. > 0 | G.3.2. > 0 || G.4.2. > 0, 1, 0))) %>%
  mutate(habitat_preferences_1027 = list(ifelse(G.3.6. > 0, 1, 0))) %>%
  mutate(habitat_preferences_8330 = list(ifelse(G.3.6. > 0 | G.4.3. > 0, 1, 0))) %>%
  mutate(habitat_preferences_1170 = list(ifelse(G.3.6. > 0 | G.3.5. > 0, 1, 0))) %>%
  mutate(bathymetry_preferred_1001 = list(ifelse(predictors$bati_mean <= -30 & predictors$bati_mean >= -200, 1, 0))) %>%
  mutate(bathymetry_preferred_1008 = list(ifelse(predictors$bati_mean <= -40 & predictors$bati_mean >= -200, 1, 0))) %>%
  mutate(bathymetry_preferred_1027 = list(ifelse(predictors$bati_mean <= -1 & predictors$bati_mean >= -20, 1, 0))) %>%
  mutate(bathymetry_preferred_1090 = list(ifelse(predictors$bati_mean <= -4 & predictors$bati_mean >= -100, 1, 0)))
...
for (i in species_habitat_codes){
  ...
  model_2 <- glm(formula = pojava~bati_norm:bathymetry_preferred_1090+
                   y_norm:lat_preferred_1090+
                   area_pct_species_1090:habitat_preferences_1090, 
                 train_df, family = "binomial")
  
   model_2 <- glm(formula = pojava~bati_norm:bathymetry_preferred_1027+
                   y_norm+
                   area_pct_species_1027:habitat_preferences_1027, 
                 train_df, family = "binomial")
  ...
  #model_3 <- lm(formula = pojava~., train_df, family = "binomial")
  #the prediction part
  #also, I need to exclude the data that is already in predictors (subset = train_df)
  predictors$predicted <- predict(model_2, newdata = predictors, subset=train_df)
